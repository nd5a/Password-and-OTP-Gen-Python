...
while True:
    choice = input(bcolors.OKGREEN + "Enter 'P' to Generate Password and Enter 'O' to Generate OTP: ").upper()
    if choice == 'P':
        while True:
            try:
        ...
        while True:
            try:
                otp_length = int(input("\tLength of OTP: "))
                break
            except ValueError:
                print(bcolors.FAIL + "Entered Value should be integer only")
                print("Restarting the program...")
        ...
        def gen_otp(length):
            ...
            print(bcolors.HEADER+ "Your OTP: ", otp)
            pyperclip.copy(otp)
            print(bcolors.OKCYAN+ "Your OTP is copied to your clipboard.")
        
        gen_otp(otp_length)
    
    elif choice=='0':
        print(bcolors.OKBLUE + "Thank you for using this program.")
        break
    
    else:
        print(bcolors.FAIL + "Invalid choice. Please try again.")